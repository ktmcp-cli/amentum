...
import { app_api_endpoints_JB2008_sample_atmosphere, app_api_endpoints_NRLMSISE00_sample_atmosphere, app_api_wfs_endpoints_WFS_get_values } from './api.js';
...
function requireAuth() {
  if (!isConfigured()) {
    ...
    console.log(chalk.cyan('  amentum config set --api-key YOUR_API_KEY'));
    process.exit(1);
  }
}
...
program
  .name('amentum')
  .description(chalk.bold('Atmosphere API CLI') + ' - Production-ready CLI')
  .version('1.0.0');
...
configCmd
  ...
  .action(() => {
    ...
    console.log(chalk.bold('\nAtmosphere API CLI Configuration\n'));
...
program
  ...
  .action(async (options) => {
    ...
    try {
      const data = await withSpinner('Making API call...', () => app_api_endpoints_JB2008_sample_atmosphere());
